...
/* The following replaces the above line; ARSW3 instead of LRSW3 so it
 * works with negative coordinates.
 */
	asm(" MOVH    0x10(%ap),%r0	"); /* r0 = p.x */
	asm(" MOVW    %r0,%r1		"); /* r1 = p.x */
	asm(" ADDW2   %r4,%r0		"); /* r0 = p.x+DX1 */
	asm(" ARSW3   &0x5,%r0,%r0	"); /* r0>>5, maintain sign */
	asm(" ARSW3   &0x5,%r1,%r1	"); /* p.x>>5, maintain sign */
	asm(" SUBW3   %r1,%r0,0x10(%fp)	"); /* w=right edge word - left edge */
	asm(" NOP     ");
...
/*		} while (--h > 0);		*/
	...
	case F_STORE | NOSHIFT:
		if (w > 25)
			goto widestore;
		i = 25 - w;	/* number of instructions to skip */
		m = (i << 2) + (i << 1); /* 6 bytes per instruction */
		
		asm(" LLSW3 &0x2,0x10(%fp),%r0	"); /* r0 = 4*(w + 1) */
		asm(" ADDW2 &4,%r0		"); /*  (offset to right edge) */
		asm(" MOVAW B_FS_N,%r1		"); /* r1=jump address into */
		asm(" ADDW2 %r4,%r1		"); /*  unrolled loop (r4==m) */
		m = h;		/* m is free reg, use it as height */
		asm(" MOVW  0x20(%fp),%r2	"); /* r2 = mask1 */
		asm(" PUSHW %ap			"); /* save fp and ap */
		asm(" MOVW  0x24(%fp),%ap	"); /* ap = mask2 */
		asm(" PUSHW %fp			");
		asm(" MOVW  %r1,%fp		"); /* fp = jump address */

		do{
		asm(" XORW3 0(%r7),0(%r8),%r1	"); /* do left edge */
		asm(" ANDW2 %r2,%r1		");
		asm(" XORW2 %r1,0(%r7)		");
		asm(" ADDW2   %r0,%r8		"); /* dest,source point */
		asm(" ADDW2   %r0,%r7		"); /*  to right edge */

		asm(" JMP     0(%fp)		"); /* copy line from left */
		asm("B_FS_N:			"); /*  to right edge */
		asm(" MOVW    -0x64(%r8),-0x64(%r7)");
		asm(" MOVW    -0x60(%r8),-0x60(%r7)");
		asm(" MOVW    -0x5c(%r8),-0x5c(%r7)");
		asm(" MOVW    -0x58(%r8),-0x58(%r7)");
		asm(" MOVW    -0x54(%r8),-0x54(%r7)");
		asm(" MOVW    -0x50(%r8),-0x50(%r7)");
		asm(" MOVW    -0x4c(%r8),-0x4c(%r7)");
		asm(" MOVW    -0x48(%r8),-0x48(%r7)");
		asm(" MOVW    -0x44(%r8),-0x44(%r7)");
		asm(" MOVW    -0x40(%r8),-0x40(%r7)");
		asm(" MOVW    -0x3c(%r8),-0x3c(%r7)");
		asm(" MOVW    -0x38(%r8),-0x38(%r7)");
		asm(" MOVW    -0x34(%r8),-0x34(%r7)");
		asm(" MOVW    -0x30(%r8),-0x30(%r7)");
		asm(" MOVW    -0x2c(%r8),-0x2c(%r7)");
		asm(" MOVW    -0x28(%r8),-0x28(%r7)");
		asm(" MOVW    -0x24(%r8),-0x24(%r7)");
		asm(" MOVW    -0x20(%r8),-0x20(%r7)");
		asm(" MOVW    -0x1c(%r8),-0x1c(%r7)");
		asm(" MOVW    -0x18(%r8),-0x18(%r7)");
		asm(" MOVW    -0x14(%r8),-0x14(%r7)");
		asm(" MOVW    -0x10(%r8),-0x10(%r7)");
		asm(" MOVW    -0xc(%r8),-0xc(%r7)  ");
		asm(" MOVW    -0x8(%r8),-0x8(%r7)  ");
		asm(" MOVW    -0x4(%r8),-0x4(%r7)  ");
		asm(" XORW3   0(%r7),0(%r8),%r1	"); /* do right edge */
		asm(" ANDW2   %ap,%r1		"); /* ap has mask2 */
		asm(" XORW2   %r1,0(%r7)	");
		asm(" ADDW2   %r6,%r8		"); /* source,dest point to */
		asm(" ADDW2   %r5,%r7		"); /* left edge of next line */
	} while (--m > 0);
		asm(" POPW	%fp		");
		asm(" POPW	%ap		");
	break;
